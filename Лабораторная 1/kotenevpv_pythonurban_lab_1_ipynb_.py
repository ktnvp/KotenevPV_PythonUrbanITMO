...
def task_4(list, required_value):
    ...
    for city in list:
        if city.lower() == required_value.lower():
            count_values += 1
           # print(count_values) ## Не справился с методом count. Попробовал применить такую конструкцию, которая выполняла аналогичную задачу в самостоятельной работе в лекции 2, но она не работает здесь, будучи упакованной в функцию. Не понимаю, почему
