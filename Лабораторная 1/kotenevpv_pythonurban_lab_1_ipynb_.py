# -*- coding: utf-8 -*-
"""KotenevPV_PythonUrban_lab_1.ipynb"

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1rQRy8gXmucqBciFt34ZQIl9OYdUrZpvs
...
first_group = ["Москва", 'Владивосток', "Москва", "Санкт-Петербург"]
second_group = ['Новосибирск', "Ижевск", "Санкт-Петербург", "Москва", "Ижевск"]

def task_3(list_1, list_2):
    return [set.intersection(set(list_1), set(list_2))]
...
def task_4(list, required_value):
    count_values = 0
    for city in list:
        if city.lower() == required_value.lower():
            count_values += 1
            return(count_values) ## Не справился с методом count. Попробовал применить такую конструкцию, которая выполняла аналогичную задачу в самостоятельной работе в лекции 2, но она не работает здесь, будучи упакованной в функцию. Не понимаю, почему
...
def green_ind(list): # TODO написать функцию, которая будет высчитывать индексы озеленения для каждой территории
    for i in list: 
        green_index_i = (sum(list[i]["green_zones"] / (list[i]["territory_area"])) for i in list)
        return(green_index_i)

green_index = green_ind(list_territories)
